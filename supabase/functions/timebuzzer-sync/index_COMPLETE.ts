  ...
  try {
    ...
    if (action === "fetch-users") {
      try {
        ...
        // Extract users from account response
    ...
    if (action === "fetch-activities") {
      ...
      try {
        ...
        const requestBody: any = {
          ...
          strictDate: false,
          userIds: [], // According to API docs: https://my.timebuzzer.com/doc/ - this should be an array
        };
        
        // Add userId to the userIds array if specified
        // According to Timebuzzer API documentation, the filter endpoint accepts userIds as an array
        if (requestedUserId) {
          const userIdNum = typeof requestedUserId === 'string' ? parseInt(requestedUserId, 10) : requestedUserId;
          if (!isNaN(userIdNum)) {
            requestBody.userIds = [userIdNum];
            console.log(`Adding userId ${userIdNum} to API filter request (as userIds array)`);
          }
        }
        
        if (requestedUserId && action === "fetch-activities") {
          console.log(`Will filter by user ID ${requestedUserId} using userIds array in API request body`);
        } else {
          console.log(`No userId specified - will fetch activities for all users accessible by the API key`);
        }
        ...
        if (activities.length > 0) {
          ...
          console.log(`✅ API returned activities for ${uniqueUserIds.length} user(s): ${uniqueUserIds.join(', ')}`);
          console.log(`Activity count per user:`, uniqueUserIds.map(uid => {
            const count = activities.filter((a: any) => a.userId === uid).length;
            return `${uid}: ${count}`;
          }).join(', '));
        } else {
          console.log(`⚠️ API returned 0 activities for the specified date range`);
        }
        
        // Important: Check if API key has limited access
        // If we requested a specific user but got activities for different users, the API key may have limited permissions
        if (requestedUserId && activities.length > 0) {
          const userIdNum = typeof requestedUserId === 'string' ? parseInt(requestedUserId, 10) : requestedUserId;
          const uniqueUserIds = [...new Set(activities.map((a: any) => a.userId))];
          const hasRequestedUser = uniqueUserIds.includes(userIdNum);
          
          if (!hasRequestedUser && !isNaN(userIdNum)) {
            console.log(`⚠️ WARNING: Requested user ${userIdNum} but API returned activities for other users: ${uniqueUserIds.join(', ')}`);
            console.log(`This suggests the API key may only have access to certain users' activities.`);
            console.log(`The API key might belong to user ${uniqueUserIds[0]} and only shows that user's activities.`);
          }
        }
        ...
        if (requestedUserId && action === "fetch-activities") {
          ...
          if (activities.length === 0 && activitiesBeforeFilter > 0) {
            ...
            console.log(`  2. The API key doesn't have access to this user's activities (MOST LIKELY)`);
            console.log(`  3. The userId mapping is incorrect`);
            console.log(`💡 SOLUTION: The API key may only have access to the user who created it.`);
            console.log(`   You may need to create an API key with admin/manager permissions in Timebuzzer,`);
            console.log(`   or use a different API key that has access to all users.`);
          } else if (activities.length === 0 && activitiesBeforeFilter === 0) {
            console.log(`No activities returned from API for this date range`);
            if (requestedUserId) {
              console.log(`💡 The API key may only have access to activities for the user who created it.`);
            }
          }
        }
        ...
        const allActivitiesBeforeFilter = responseData.activities || [];
        const availableUserIds = requestedUserId && action === "fetch-activities" && activities.length === 0
          ? [...new Set(allActivitiesBeforeFilter.map((a: any) => a.userId))]
          : undefined;
        
        // Add warning message if API key seems to have limited access
        const warningMessage = requestedUserId && allActivitiesBeforeFilter.length > 0 && activities.length === 0
          ? `API key appears to only have access to user ${availableUserIds?.[0] || 'unknown'}. Requested user ${requestedUserId} not found in API response.`
          : undefined;
        ...
        return new Response(
          JSON.stringify({
            ...
            warning: warningMessage, // Warning if API key has limited access
            apiReturnedUserIds: allActivitiesBeforeFilter.length > 0 
              ? [...new Set(allActivitiesBeforeFilter.map((a: any) => a.userId))] 
              : undefined, // All user IDs that the API actually returned
          }),
          {
            status: 200,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
