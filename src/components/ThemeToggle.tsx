...
const ThemeToggle = ({ currentUser }: ThemeToggleProps) => {
  ...
  return (
    <Button
      ...
      size="sm"
      onClick={toggleTheme}
      className="h-7 w-7 sm:h-7 sm:w-7 md:h-8 md:w-8 p-0 border-gray-300 dark:border-gray-600"
      title={theme === 'light' ? 'Switch to dark mode' : 'Switch to light mode'}
    >
      {theme === 'light' ? (
        <Moon className="h-4 w-4 sm:h-3.5 sm:h-3.5 md:h-4 md:w-4 text-gray-600 dark:text-gray-300" />
      ) : (
        <Sun className="h-4 w-4 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4 text-gray-600 dark:text-gray-300" />
      )}
    </Button>
  );
};
